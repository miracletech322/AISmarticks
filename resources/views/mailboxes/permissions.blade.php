                ...
                <div class="col-md-11 col-lg-9" style="overflow-x: auto;">
                    ...
                    <table class="table">
                        <tr class="table-header-nb">
                            ...
                            @endforeach
							<th class="text-center"> {{ __('Only team conversations') }}</th>
                            <th class="text-center"> {{ __('Only unassigned conversations') }}</th>
                        </tr>
                        <fieldset id="permissions-fields">
                            @foreach ($managers as $mailbox_user)
                                <tr>
                                    ...
                                    @endforeach
									@if (!$mailbox_user->isAdmin())
										<td class="text-center"><input type="checkbox" name="managers[{{ $mailbox_user->id }}][only_team]" value="1" @if (!empty($mailbox_user->only_team)) checked="checked" @endif></td>
										<td class="text-center"><input type="checkbox" name="managers[{{ $mailbox_user->id }}][only_unassigned]" value="1" @if (!empty($mailbox_user->only_unassigned)) checked="checked" @endif></td>
									@else
										<td class="text-center"><input type="checkbox" name="managers[{{ $mailbox_user->id }}][only_team]" value="1" disabled ></td>
										<td class="text-center"><input type="checkbox" name="managers[{{ $mailbox_user->id }}][only_unassigned]" value="1" disabled ></td>
									@endif
                                </tr>
                            @endforeach
                        </fieldset>
