...
class Conversation extends Model
{
    ...
    {
		// $user = auth()->user();
		// $mailbox_user = $user->mailboxesWithSettings()->where('mailbox_id', $folder->mailbox_id)->first();
        // if (!$mailbox_user && $user->isAdmin()) {
        //     // Admin may not be connected to the mailbox yet
        //     $user->mailboxes()->attach($folder->mailbox_id);
        //     $mailbox_user = $user->mailboxesWithSettings()->where('mailbox_id', $folder->mailbox_id)->first();
        // }

		$mailbox_user = MailboxUser::where('mailbox_id', $folder->mailbox_id)->where('user_id',$user_id)->first();
		error_log('CONVERSATION Mailbox_user = '.json_encode($mailbox_user));
		$only_unassigned_applied = false;
		$only_team_applied = false;

		// $mailbox_user = User::nonDeleted()
        //     ->select(['mailbox_user.only_team', 'mailbox_user.only_unassigned'])
        ...
        // Assigned - do not show my conversations.
            ...
            $query_conversations = $folder->conversations()
                ...
                ->where('user_id', '<>', $user_id)
                ->where('state', self::STATE_PUBLISHED);
			$only_team_applied=true;
			$only_unassigned_applied=true;
		...
		// error_log('CONVERSATION Mailbox_user only_unassigned = '.json_encode(empty($mailbox_user->only_unassigned)));
		// error_log('CONVERSATION Mailbox_user only_team = '.json_encode(empty($mailbox_user->only_team)));
		...
		$query_conversations = \Eventy::filter('folder.conversations_query', $query_conversations, $folder, $user_id);
		
		if (!empty($mailbox_user->only_unassigned))
		{
			error_log('ONLY UNASSIGNED');
			error_log($query_conversations->toSql());
			$query_conversations->where(function($query) use ($user_id) {
				$query->where('conversations.user_id', '=', $user_id)->orWhere('conversations.user_id',null);
			});
			// $query_conversations->where('conversations.user_id', null);
			// $query_conversations->where('user_id', '=', $user_id)->orWhere('user_id',null);
			error_log($query_conversations->toSql());
		}
		if (!empty($mailbox_user->only_team))
		{
			$user_team_ids = \Team::getUserTeamIds($user_id);
            $user_team_ids[] = $user_id;
			error_log(json_encode($user_team_ids));
            error_log($query_conversations->toSql());
			$query_conversations->where(function($query) use ($user_team_ids) {
				$query->whereIn('conversations.user_id', $user_team_ids);
			});
			error_log($query_conversations->toSql());
		}
...
//        echo "</pre>";exit;
        ...
        if (!is_null($this->customer_cached)) {
            if (!$phones=json_decode($this->customer_cached->phones ?? '',true)) $phones=[];
            $this->customer_cached->phone=$phones[0]['value'] ?? '';
            error_log('CUSTOMER CACHED = '.json_encode($this->customer_cached));
        }
        $data = [
            'mailbox'      => $this->mailbox,
            'conversation' => $this,
    ...
    {
		error_log('changeStatus start '.$this->id);
        ...
        $this->save();
		error_log('changeStatus saved '.$this->id);
