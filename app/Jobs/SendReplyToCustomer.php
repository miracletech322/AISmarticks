...
class SendReplyToCustomer implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
	    foreach ($reply_mail->threads as $tk=>$thread)
	    {
		// \Log::error('MAILLOG: '.json_encode($thread->body));
		if (preg_match('/[\\x{0590}-\\x{083F}\\x{08A0}-\\x{08FF}\\x{FB1D}-\\x{FDFF}\\x{FE70}-\\x{FEFF}]+/uis',$thread->body))
		{
			// \Log::error('MAILLOG: '.json_encode(preg_replace('/<div/uis','<div style="direction:rtl;"',$thread->body)));
			$reply_mail->threads[$tk]->body='<div style="direction:rtl;">'.$thread->body.'</div>';
		}
	    }
            Mail::to($to)
                ->cc($cc_array)
