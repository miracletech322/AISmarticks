...
class MailboxesController extends Controller
{
    ...
    public function createSave(Request $request)
    {
        ...
        $maxMailboxes = LicenseLimit::getLimits();
        if (!empty($maxMailboxes->mailbox) && Mailbox::count() >= $maxMailboxes->mailbox) {
            \Session::flash('flash_error_floating', __("You reached the maximum mailboxes. Please contact the administrator"));
            return redirect()->route('mailboxes.create');
        }
    ...
    public function permissions($id)
    {
        ...
        $managers = User::nonDeleted()
            ->select(['users.*', 'mailbox_user.hide', 'mailbox_user.only_team', 'mailbox_user.only_unassigned', 'mailbox_user.access'])
    ...
    public function permissionsSave($id, Request $request)
    {
        ...
        foreach ($admins as $admin) {
            ...
            $mailbox_user->settings->only_team = (isset($request->managers[$admin->id]['only_team']) ? (int)$request->managers[$admin->id]['only_team'] : false);
            $mailbox_user->settings->only_unassigned = (isset($request->managers[$admin->id]['only_unassigned']) ? (int)$request->managers[$admin->id]['only_unassigned'] : false);
        ...
        foreach ($mailbox_users as $mailbox_user) {
            ...
            $mailbox_with_settings->settings->only_team = (isset($request->managers[$mailbox_user->id]['only_team']) ? (int)$request->managers[$mailbox_user->id]['only_team'] : false);
            $mailbox_with_settings->settings->only_unassigned = (isset($request->managers[$mailbox_user->id]['only_unassigned']) ? (int)$request->managers[$mailbox_user->id]['only_unassigned'] : false);
