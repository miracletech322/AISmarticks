...
class Thread extends Model
{
    ...
    public function updateSendStatusData($new_data)
    {
        if ($new_data) {
            ...
            if (!empty($send_status_data['msg'])) {
				error_log('BEFORE SANITIZE = '.json_encode($send_status_data['msg']));
                $send_status_data['msg'] = \MailHelper::sanitizeSmtpStatusMessage($send_status_data['msg']);
            	error_log('AFTER SANITIZE = '.json_encode($send_status_data['msg']));
            }
    ...
    public static function createExtended($data = [], $conversation = null, $customer = null, $update_conv = true)
    {
        ...
        $user_id = $data['created_by_user_id'] ?? $data['user_id'] ?? 4;
