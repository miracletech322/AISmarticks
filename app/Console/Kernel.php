...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
		...
		$schedule->command('freescout:billing-report')
            ->cron('2 0 * * *')
            ->withoutOverlapping();
		...
		$schedule->command('freescout:update-current-folder-counters')
			->cron('* * * * *')
			->withoutOverlapping();
...
//                ->cron((int)($crc % 59).' '.(int)($crc % 23).' * * *');
		...
		$schedule->call(function () {
			VoipeSmsTicketsServiceProvider::listenIncomingMessage();
		})->name('module_voipesmstickets_listen_sms')->withoutOverlapping();
    }
