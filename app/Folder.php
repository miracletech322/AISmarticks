...
class Folder extends Model
{
    ...
    {
		error_log('FOLDER Update Counters Now '.$this->getTypeName().' '.$this->id);
		if (\Eventy::filter('folder.update_counters', false, $this)) {
            return;
        }
		$mailbox_user = MailboxUser::where('mailbox_id', $this->mailbox_id)->where('user_id',$this->user_id)->first();
		error_log('FOLDER Mailbox_user = '.json_encode($mailbox_user));
		if ($this->type == self::TYPE_MINE && $this->user_id) {
			...
			if (!empty($mailbox_user->only_unassigned))
			{
				...
				$this->active_count->where(function($query) use ($tmp_user_id) {
					$query->where('conversations.user_id', '=', $this->user_id)->orWhere('conversations.user_id',null);
				});
				error_log($this->active_count->toSql());
			}
			if (!empty($mailbox_user->only_team))
			{
				$user_team_ids = \Team::getUserTeamIds($this->user_id);
				$user_team_ids[] = $this->user_id;
				$this->active_count->where(function($query) use ($user_team_ids) {
					$query->whereIn('conversations.user_id', $user_team_ids);
				});
				error_log($this->active_count->toSql());
			}
            $this->active_count=$this->active_count->count();
			...
			{
				$tmp_user_id=$this->user_id;
				$this->total_count->where(function($query) use ($tmp_user_id) {
					$query->where('conversations.user_id', '=', $this->user_id)->orWhere('conversations.user_id',null);
				});
				error_log($this->total_count->toSql());
			}
			if (!empty($mailbox_user->only_team))
			{
				$user_team_ids = \Team::getUserTeamIds($this->user_id);
				$user_team_ids[] = $this->user_id;
				$this->total_count->where(function($query) use ($user_team_ids) {
					$query->whereIn('conversations.user_id', $user_team_ids);
				});
				error_log($this->total_count->toSql());
			}
    ...
    public function getCount($folders = [])
    {
        ...
        if ($counter == self::COUNTER_TOTAL || $this->type == self::TYPE_STARRED || $this->type == self::TYPE_DRAFTS) {
			...
			if ($user = auth()->user())
			{
				$mailbox_user = MailboxUser::where('mailbox_id', $this->mailbox_id)->where('user_id',$user->id)->first();
				if (!empty($mailbox_user->only_unassigned))
				{
					$tmp_user_id=$user->id;
					if ($this->type == self::TYPE_MINE && $this->user_id) {
						$total_count = Conversation::where('user_id', $this->user_id)
							->where('mailbox_id', $this->mailbox_id)
							->where('state', Conversation::STATE_PUBLISHED)
							->where(function($query) use ($tmp_user_id) {
								$query->where('conversations.user_id', '=', $this->user_id)->orWhere('conversations.user_id',null);
							})->count();
					} elseif ($this->type == self::TYPE_DELETED) {
						$total_count = $this->conversations()
						...
						// Conversation are connected to folder via conversation_folder table.
						// Drafts.
						$total_count = ConversationFolder::where('conversation_folder.folder_id', $this->id)
							->join('conversations', 'conversations.id', '=', 'conversation_folder.conversation_id')
							->where(function($query) use ($tmp_user_id) {
								$query->where('conversations.user_id', '=', $this->user_id)->orWhere('conversations.user_id',null);
							})->count();
				...
				else if (!empty($mailbox_user->only_team))
				{
					$user_team_ids = \Team::getUserTeamIds($user->id);
					$user_team_ids[] = $user->id;
					if ($this->type == self::TYPE_MINE && $this->user_id) {
						$total_count = Conversation::where('user_id', $this->user_id)
							->where('mailbox_id', $this->mailbox_id)
							->where('state', Conversation::STATE_PUBLISHED)
							->where(function($query) use ($user_team_ids) {
								$query->whereIn('conversations.user_id', $user_team_ids);
							})->count();
					} elseif ($this->type == self::TYPE_DELETED) {
						$total_count = $this->conversations()
    ...
    public function getActiveCount($folders = [])
    {
		...
		if ($user = auth()->user())
		{
			...
			if (!empty($mailbox_user->only_unassigned))
			{
				$tmp_user_id=$user->id;
				if ($this->type == self::TYPE_MINE && $this->user_id) {
					$active_count = Conversation::where('user_id', $this->user_id)
						->where('mailbox_id', $this->mailbox_id)
						->where('state', Conversation::STATE_PUBLISHED)
						->where('status', Conversation::STATUS_ACTIVE)
						->where(function($query) use ($tmp_user_id) {
					...
					$active_count = $this->conversations()
						->where('state', Conversation::STATE_DELETED)
						->where(function($query) use ($tmp_user_id) {
							$query->where('conversations.user_id', '=', $this->user_id)->orWhere('conversations.user_id',null);
						})->count();
       			} elseif ($this->isIndirect()) {
					$active_count = ConversationFolder::where('conversation_folder.folder_id', $this->id)
						->join('conversations', 'conversations.id', '=', 'conversation_folder.conversation_id')
						->where(function($query) use ($tmp_user_id) {
							$query->where('conversations.user_id', '=', $this->user_id)->orWhere('conversations.user_id',null);
						})->count();
