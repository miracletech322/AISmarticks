...
class TelegramDriver extends HttpDriver
{
    ...
    public function getUserPhoto($userId)
    {
        ...
        $response = $this->http->post($this->buildApiUrl('getUserProfilePhotos'), [], $parameters);
        $responseData = json_decode($response->getContent(), true);

        if ($response->getStatusCode() !== 200) {
            throw new TelegramException('Error retrieving user profile photos: '.$responseData['description']);
        }
        ...
        $response = $this->http->post($this->buildApiUrl('getFile'), [], $parameters);
        $responseData = json_decode($response->getContent(), true);

        if ($response->getStatusCode() !== 200) {
            throw new TelegramException('Error retrieving user profile photo file ID: '.$responseData['description']);
        }
        ...
        $photoUrl = 'https://api.telegram.org/file/bot'.$this->config->get('token').'/'.$filePath;
