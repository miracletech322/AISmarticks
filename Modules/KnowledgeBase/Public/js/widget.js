 ...
 */
if (typeof(SmarticksW) != "undefined" && SmarticksW && typeof(SmarticksW.s) != "undefined") {

    SmarticksW.show = function (btn, minimized) {
    	...
    	if (!iframe) {
	        ...
	        iframe.src = SmarticksW.form_url;
    ...
    SmarticksW.init = function () {
        ...
        // Determine form url
        var script = document.getElementById('smarticks-w');
        if (script) {
            SmarticksW.form_url = script.src.replace(/modules\/knowledgebase\/.*/, '');
            SmarticksW.form_url += 'knowledgebase/widget/form/'+SmarticksW.s.id;
            SmarticksW.form_url += '?'+Object.keys(SmarticksW.s)
                .map(function(k) {return encodeURIComponent(k) + '=' + encodeURIComponent(SmarticksW.s[k]);})
                .join('&');
        }

        SmarticksW.listenForEvent('fsw.minimize', function() {  SmarticksW.minimize(); });
        SmarticksW.listenForEvent('fsw.newmessage', function() {  SmarticksW.newmessage(); });

        // Show minimized
        //SmarticksW.show(btn, true);
    };

    SmarticksW.listenForEvent = function (event_name, callback) {
