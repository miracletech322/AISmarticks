    ...
    public function hooks()
    {
        ...
        \Eventy::addAction('search.conversations_tab_append', function($filters, $count) {
            if (request()->mode != "customers" && $count) {
                ...
                // }
                ?>
                    <i class="glyphicon glyphicon-download-alt ec-search-export fs-trigger-modal" data-toggle="tooltip" title="<?php echo __('Export Conversations') ?>" data-remote="<?php echo route('exportconversations.ajax_html', ['action' => 'export', 'f' => $f]) ?>" data-modal-title="<?php echo __('Export Conversations') ?> (CSV)" data-modal-no-footer="true"></i>
                <?php
            }
        }, 20, 2);
        ...
        \Eventy::addAction('reports.filters_button_append', function() {
            ?>
                <div class="rpt-filter" data-toggle="tooltip" title="<?php echo __('Export Conversations') ?>">
                    <button class="btn btn-primary fs-trigger-modal" id="rpt-btn-export" data-remote="<?php echo route('exportconversations.ajax_html', ['action' => 'export']) ?>" data-modal-title="<?php echo __('Export Conversations') ?> (CSV)" data-modal-no-footer="true"><i class="glyphicon glyphicon-download-alt"></i></button>
                </div>
            <?php
        });
    }
