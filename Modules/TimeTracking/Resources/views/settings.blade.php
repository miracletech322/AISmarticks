<form class="form-horizontal margin-top margin-bottom" method="POST" action="" id="slack_form">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            ...
            <div class="control-group">
                <label class="radio" for="mode_{{ TimeTracking::MODE_MANUAL }}">
                    <input type="radio" name="settings[timetracking.mode]" id="mode_{{ TimeTracking::MODE_MANUAL }}" value="{{ TimeTracking::MODE_MANUAL }}" @if (old('settings[timetracking.mode]', $settings['timetracking.mode']) == TimeTracking::MODE_MANUAL) checked="checked" @endif> {{ __('The user starts and stops the timer manually.') }}
                </label>
            </div>
        </div>
    </div>
    ...
    <div class="form-group">
        <label for="allow_add" class="col-sm-2 control-label">{{ __('Allow to Add Time Manually') }}</label>

        <div class="col-sm-6">
            <div class="controls">
                <div class="onoffswitch-wrap">
                    <div class="onoffswitch">
                        <input type="checkbox" name="settings[timetracking.allow_add]" value="1" id="allow_add" class="onoffswitch-checkbox" @if (old('settings[timetracking.allow_add]', $settings['timetracking.allow_add']))checked="checked"@endif >
                        <label class="onoffswitch-label" for="allow_add"></label>
                    </div>
                </div>
            </div>
            <p class="form-help">
                {{ __('Allow users to submit time any time.') }}
            </p>
        </div>
    </div>
