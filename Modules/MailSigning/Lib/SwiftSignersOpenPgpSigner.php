...
use Modules\MailSigning\Lib\Gnupg;
...
EOT;
    ...
    protected function initGNUPG()
    {
        if (!extension_loaded('gnupg')) {
            throw new \Swift_SwiftException('Module requires the GnuPG Extension');
        }
        ...
        if (!$this->gnupgHome) {
            throw new \Swift_SwiftException('GnuPG path is not set');
        }

        if (!file_exists($this->gnupgHome)) {
            throw new \Swift_SwiftException('GnuPG home path does not exist: '.$this->gnupgHome);
        }
    ...
    {
        if (!empty($this->keyPassphrases[$keyFingerprint])) {
