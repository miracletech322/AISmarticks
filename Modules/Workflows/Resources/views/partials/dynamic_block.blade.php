                                ...
                                @foreach ($row_config as $row_info)
                                    @foreach ($row_info['items'] as $row_item_key => $row_item)
                                        <span class="wf-block-value wf-block-value-{{ $row_item_key }} hidden" @if (!empty($row_item['values_visible_if'])) data-values-visible-if="{{ implode(',', $row_item['values_visible_if'])}}" @endif>
                                            ...
                                            @elseif (in_array($row_item_key, ['workflow_triggered', 'run_workflow']))
                                                <select class="form-control" name="{{ $mode }}[{{ $and_i }}][{{ $row_i }}][value]" disabled>
                                                        @foreach ($row_item['values'] as $value => $title)
                                                        <option value="{{ $value }}" @if (isset($row_value) && $value == $row_value) selected @endif><span style="color:red">{{ $value }}: </span> {{ $title }}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            @elseif ((!isset($row_item['values']) || !is_array($row_value)) && $row_item_key != 'office_hours')
                                                <input class="ddd form-control {{ $row_item['values_classes'] ?? '' }}" type="{{ $row_item['values_type'] ?? 'text' }}" value="{{ $row_value ?? '' }}" name="{{ $mode }}[{{ $and_i }}][{{ $row_i }}][value]" disabled/>
                                            @endif
                                        </span>
                                    @endforeach
                                @endforeach
