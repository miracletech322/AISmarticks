...
class WorkflowsController extends Controller
{
    ...
    public function createSave($mailbox_id, Request $request)
    {
        ...
        $workflowLimit = LicenseLimit::where(['id' => 1])->get()->toArray()[0]['workflow'];
        $countWorkflows = \DB::table('workflows')
					...
					->count();
		error_log('Workflowlimit = '.json_encode($workflowLimit));
		error_log('CountWorkflows = '.json_encode($countWorkflows));
		if (!empty($workflowLimit) && $countWorkflows >= $workflowLimit) {
            session()->flash('max_workflows', 'Max workflows!');
            return redirect()->route('mailboxes.workflows.create', ['mailbox_id' => $mailbox_id])
                ->withInput();
        }
		else if (empty($workflowLimit))
		{
			session()->flash('max_workflows', 'Max workflows!');
            return redirect()->route('mailboxes.workflows.create', ['mailbox_id' => $mailbox_id])
                ->withInput();
