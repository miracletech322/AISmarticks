...
class Workflow extends Model
{
    ...
    public static function conditionsConfig($mailbox_id)
    {
        ...
        self::$conditions_config[$mailbox_id] = [
                ...
                'items' => [
                    ...
                    'new_or_reply' => [
                        ...
                        'operators' => [
                            ...
                            'reply_user' => __('User replied'),
                            'reply_customer' => __('Customer replied'),
            ...
            'dates' => [
                ...
                'items' => [
                    ...
                    'office_hours' => [
                        ...
                        'operators' => [
                            'matches_working_hours' => __('Matches working hours'),
                            'is_not_matches_working_hours' => __('Is not matches working hours'),
                        ],
                        'triggers' => [
                            'conversation.created_by_user',
                            'conversation.user_forwarded',
            ...
            'workflow' => [
                ...
                'items' => [
                    'workflow_triggered' => [
                        'title' => __('Workflow triggered'),
                        'values' => self::where(['active' => 1, 'mailbox_id' => $mailbox_id])->pluck('name', 'id')->toArray(),
                        'triggers' => [
                            'workflow.all'
                        ]
                    ],
        ...
        foreach ($conditions as $list_i => $list) {
            foreach ($list as $condition_i => $condition) {
                ...
                if (empty($condition['type'])
                    || (empty($condition['operator']) && !in_array($condition['type'], ['customer_phone_number', 'workflow_triggered']))
                    || (!isset($condition['value']) 
                            && $config 
                            && $condition['type'] != 'office_hours'
        ...
        if ($action) {
            ...
            foreach ($this->conditions as $ands) {
                foreach ($ands as $row) {
                    ...
                    if ($row['type'] == 'workflow_triggered') {
                        $valid_trigger = true;
                        break 2;
                    }
            ...
            foreach ($ands as $row) {
                ...
                switch ($row['type']) {
                    ...
                    case 'customer_phone_number':
                        if (empty($row['value']) || empty($conversation->customer_id)) {
                            continue 2;
                        }

                        $customer = Customer::findOrFail($conversation->customer_id);
                        if ($customer) {
                            $customer = $customer->getPhones();
                            if (empty($customer[0]['value'])) {
                                continue 2;
                            }else {
                                $searchPhone = $row['value'];
                                $phone = $customer[0]['value'];
                    ...
                    case 'new_or_reply':
                        ...
                        $actions_by_operator = [
                            ...
                            'reply_user' => ['conversation.user_replied'],
                            'reply_customer' => ['conversation.customer_replied'],
					...
					// People.
                    case 'reply_last_customer':
						
						$found=false;
						$threads = $conversation->getThreads(null, null, [Thread::TYPE_CUSTOMER, Thread::TYPE_NOTE, Thread::TYPE_MESSAGE]);
						foreach ($threads as $thread) if (!$found)
						{
							if ($thread->source_via == Thread::PERSON_CUSTOMER) 
							{
								$or_true = self::compareText($thread->body, $value, $operator);
								$found=true;
							}
						}
                        break;
                    ...
                    case 'office_hours':
                        ...
                        if (empty($schedule) || empty($conversation->created_at)) {
                            continue 2;
                        }

                        $convDate = new \DateTime($conversation->created_at);
                        $convDayOfWeek = $convDate->format('N');
                        $convTime = $convDate->format('H:i');

                        if ($convDayOfWeek == 7)
                            $convDayOfWeek = 0;

                        if ($operator == 'matches_working_hours') {
                            $or_true = self::isTimeWithinRange($convDayOfWeek, $convTime, $schedule);
                        } else {
                            // Is not matches working hours
                            $or_true = !self::isTimeWithinRange($convDayOfWeek, $convTime, $schedule);
                        }
                        break;
                    ...
                    case 'workflow_triggered':
                        $WSW = self::getWSW();
                        if (in_array($this->id, array_keys($WSW)) && $action != 'workflow.workflow_triggered') {
                            continue 2;
                        }
    ...
    public function performActions($conversation/*, $mark_processed = true*/)
    {
        ...
        if (!empty($runWorkflows)) {
            foreach ($runWorkflows as $workflowId) {
                $actionWorkflow = self::where([
                        'id' => $workflowId,
                        'active' => true
                    ])
                    ->firstOrFail();
                ...
                if (!empty($actionWorkflow)) {
                    $actionWorkflow->performActions($conversation);
                }
            }
        }

        // get Workflows that Start other Workflows - WSW
        $WSW = self::getWSW();
        if (self::in_array_recursive($this->id, $WSW)) { // Trigger an action if the desired workflow is running
            $WSWID = self::array_search_recursive((string)$this->id, $WSW); // Достаю ID Workflow, которое нужно запустить
            $thisWorkflow = Workflow::where([
                    'id' => $WSWID,
                    'active' => true
                ])
                ->firstOrFail();
            if (!empty($thisWorkflow) && $thisWorkflow->checkConditions($conversation, 'workflow.workflow_triggered')) {
                $thisWorkflow->performActions($conversation);
            }
        }
    ...
    public function validateConditions($errors)
    {
        ...
        foreach ($this->conditions as $and_i => $ands) {
            foreach ($ands as $or_i => $condition) {
                $has_error = false;
                if (empty($condition['type']) || (empty($condition['operator']) && !in_array($condition['type'], ['customer_phone_number', 'workflow_triggered']))) {
                    $has_error = true;
                }
                if (!$has_error) {
    ...
    public static function isTimeWithinRange($day, $time, $schedule) {
        // Преобразование времени в объект DateTime для сравнения
        ...
        // Проверка на наличие расписания для указанного дня
        if (!isset($schedule[$day])) {
            return false;
        }

        $daySchedule = $schedule[$day][0];
        ...
        // Преобразование интервалов времени в объекты DateTime
        ...
        if ($toTime < $fromTime) {
            // Если текущее время между "от" и 23:59 или между 00:00 и "до", то попадает в интервал
    ...
    public static function getWSW()
    {
        $result = Workflow::select('id', \DB::raw("JSON_ARRAYAGG(JSON_UNQUOTE(JSON_EXTRACT(element, '$.value'))) as cond"))
            ->where('active', 1)
            ->whereRaw("
                JSON_CONTAINS(
                    JSON_EXTRACT(conditions, '$[*][*]'),
                    JSON_OBJECT('type', 'workflow_triggered')
                )
            ")
            ->crossJoin(\DB::raw("
                JSON_TABLE(
            ...
            "))
            ->whereRaw("
                JSON_EXTRACT(element, '$.type') = 'workflow_triggered'
            ")
            ->groupBy('id')
            ->get()
            ->pluck('cond', 'id')
            ->map(function($item) {
                return json_decode($item, true); // Декодируем JSON в массив
            })
            ->toArray();
    ...
    public static function in_array_recursive($needle, $haystack, $strict = false) {
        foreach ($haystack as $item) {
            if (is_array($item)) {
                if (self::in_array_recursive($needle, $item, $strict)) {
                    return true;
                }
    ...
    public static function array_search_recursive($needle, $haystack, $strict = false) {
        foreach ($haystack as $key => $value) {
            if (is_array($value)) {
                $result = self::array_search_recursive($needle, $value, $strict);
                if ($result !== false) {
                    return $key; // возвращаем ключ верхнего уровня, где найдено совпадение
                }
            } elseif (($strict ? $value === $needle : $value == $needle)) {
                return $key; // возвращаем ключ текущего уровня
            }
        }
        return false;
    }
