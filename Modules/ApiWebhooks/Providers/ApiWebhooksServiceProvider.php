...
class ApiWebhooksServiceProvider extends ServiceProvider
{
    ...
    public function hooks()
    {
        ...
        \Eventy::addFilter('settings.after_save', function($response, $request, $section, $settings) {
            ...
            if ($request->action == 'add') {
                if (!empty($request->url) && !empty($request->events)) {
					...
					$webhook->headers = ($request->headers_type=='custom');
					$webhook->headers_text = $request->headers_text;
					$webhook->save();
            ...
            if ($request->action == 'save_webhook') {
                if (!empty($request->id) && !empty($request->url) && !empty($request->events)) {
                    error_log('DATA = '.json_encode($request->headers_type));
        ...
        \Eventy::addAction('conversation.created_by_user', function($conversation, $thread) {
            error_log('Webhook Action user created');
        ...
        \Eventy::addAction('conversation.created_by_customer', function($conversation, $thread) {
            error_log('Webhook Action customer created');
			self::maybeRunWebhook('convo.created', $conversation);
        }, 20, 2);

        // Conversation assigned.
        \Eventy::addAction('conversation.user_changed', function($conversation, $by_user) {
            error_log('conversation.user_changed ApiWebhooks start');
			if (!$conversation->user_id) {
				error_log('conversation.user_changed ApiWebhooks stop');
			    return;
            }
        ...
        \Eventy::addAction('conversation.deleting', function($conversation) {
            self::maybeRunWebhook('convo.deleted_forever', $conversation);
        });

        // Conversation bulk deleted forever.
        \Eventy::addAction('conversations.before_delete_forever', function($conversation_ids) {
            $event_exists = false;

            $webhooks = self::getWebhooks();
            foreach ($webhooks as $webhook) {
                if (in_array('convo.deleted_forever', $webhook->events)) {
                    $event_exists = true;
                    break;
                }
            }
        ...
        \Eventy::addAction('conversation.state_changed', function($conversation, $user, $prev_state) {
            if ($prev_state == Conversation::STATE_DELETED && $conversation->state == Conversation::STATE_PUBLISHED) {
                self::maybeRunWebhook('convo.restored', $conversation);
            }
        }, 20, 3);
        ...
        \Eventy::addAction('conversation.status_changed', function($conversation, $user, $changed_on_reply) {
            error_log('conversation.status_changed ApiWebhooks start');
			if ($changed_on_reply) {
                error_log('conversation.status_changed ApiWebhooks stop');
				return false;
            }
        ...
        \Eventy::addAction('conversation.customer_replied', function($conversation, $thread) {
			error_log('Webhook Action customer replied');
        ...
        \Eventy::addAction('conversation.after_subject', function($conversation, $mailbox) {
            if ($conversation->source_type == Conversation::SOURCE_TYPE_API)
			{
				$tags = [__('API')];
				echo \View::make('apiwebhooks::partials/subject_tags', ['tags' => $tags])->render();
			}
        }, 20, 2);
        ...
        \Eventy::addAction('conversations_table.before_subject', function($conversation) {   
			if ($conversation->source_type == Conversation::SOURCE_TYPE_API)
			{
                ...
                echo \View::make('apiwebhooks::partials/conversation_list_tag', ['tag' => __('API')])->render();
                echo '</span>&nbsp;';
			}
        }, 200, 1);
    }
    ...
    {
		error_log('Webhook maybeRunWebhook '.json_encode($event));
        ...
        foreach ($webhooks as $webhook) {
            ...
            if (in_array($event, $webhook->events)) {
				error_log('Webhook background run '.json_encode($data));
    ...
    public static function formatEntity($entity, $full = true, $entity_type = '', $extra_data = [])
    {
        ...
        if ($entity instanceof Conversation) {
            ...
            $result = [
                ...
                'assignee' => ($entity->user_id && $entity->user ? self::formatEntity($entity->user, true) : null),
                'createdBy' => ($entity->source_via == Conversation::PERSON_USER ? self::formatEntity($entity->created_by_user, true) : self::formatEntity($entity->created_by_customer, true)),
                ...
                'closedByUser' =>  ($entity->status == Conversation::STATUS_CLOSED && $entity->closed_by_user_id ? self::formatEntity($entity->closed_by_user, true) : null),
                ...
                'customer' => ($entity->customer_id && $entity->customer ? self::formatEntity($entity->customer, true) : null),
                // 'customFields'
                '_embedded' => $embedded,
                // '_links' => [
        ...
        if ($entity instanceof Thread) {
            $result = [
                ...
                'action' => [
                    ...
                    'text' => $entity->conversation ? strip_tags($entity->getActionDescription($entity->conversation->number, false)) : '',
                    'associatedEntities' => [
                        // todo
                    ],
                ],
		...
		error_log('FormatEntity Result ('.json_encode($full).') = '.json_encode($result));
