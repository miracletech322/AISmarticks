...
<div class="form-group cs-signature-block">
    <div class="col-md-9 col-sm-offset-2">
        ...
        <textarea class="form-control cs-signature-text" id="cs_signature_text_{{ $signature->id }}" name="cs_signature_text{{ $signature_suffix }}[{{ $signature->id }}]" rows="8">{{ \Helper::stripDangerousTags($signature->text) ?: \App\Mailbox::DEFAULT_SIGNATURE }}</textarea>
    </div>
</div>