<form class="form-horizontal margin-top margin-bottom" method="POST" action="">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            ...
            <p class="form-help">
                {{ __('If you change the Jira Base URL later, previously created links in Smarticks leading to Jira issues will stop working.') }}
            </p>
        </div>
    </div>
        ...
        <div class="col-sm-6">
            <div class="control-group">
                <label class="radio" for="jira_on_resolve_1">
                    <input type="radio" name="settings[jira.on_resolve]" value="{{ \Jira::ON_RESOLVE_ACTIVATE }}" id="jira_on_resolve_1" @if (!$settings['jira.on_resolve'] || $settings['jira.on_resolve'] == \Jira::ON_RESOLVE_ACTIVATE) checked="checked" @endif> {{ __('Change Smarticks conversation status to Active and add a note when linked Jira issue is resolved.') }}
                </label>
                <label class="radio" for="jira_on_resolve_2">
                    <input type="radio" name="settings[jira.on_resolve]" value="{{ \Jira::ON_RESOLVE_NOTE }}" id="jira_on_resolve_2" @if ($settings['jira.on_resolve'] == \Jira::ON_RESOLVE_NOTE) checked="checked" @endif> {{ __('Add a note to Smarticks conversation informing that linked Jira issue was resolved.') }}
                </label>
            </div>
        </div>
    </div>
