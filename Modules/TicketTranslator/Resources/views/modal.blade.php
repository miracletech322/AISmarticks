<div class="form-horizontal">
	...
	<div class="form-group">
	    ...
	    <div class="col-sm-10">
			<select type="text" class="form-control input-sized-lg input-translate-into">
		        @foreach ($languages as $lang_code => $lang_info)
		            <option value="{{ $lang_code }}" @if ($into == $lang_code)) selected @endif>{{ $lang_info['name_en'] }}</option>
		        @endforeach
		    </select>
		</div>
	</div>
	...
	<div class="form-group">
		<div class="col-sm-10 col-sm-offset-2">
			<button class="btn btn-primary ttr-translate" data-loading-text="{{ __('Translate') }}…">{{ __('Translate') }}</button>
		</div>
	</div>
</div>