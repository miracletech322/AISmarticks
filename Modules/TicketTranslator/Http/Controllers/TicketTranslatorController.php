...
class TicketTranslatorController extends Controller
{
    ...
    public function modal($thread_id)
    {
        ...
        $into = auth()->user()->getLocale();
        $user_into = \Option::get('tickettranslator.into');
        if (!empty($user_into) && is_array($user_into) && !empty($user_into[auth()->id()])) {
            $into = $user_into[auth()->id()];
        }
        ...
        switch ($request->action) {
            case 'translate':
                ...
                if (!$response['msg']) {
                    ...
                    $thread->body = preg_replace("#(<img[^<>]+src=[\"'])data:image/([^;]+);base64,([^\"']+)([\"'])#", "$1$4", $thread->body);
                    ...
                    if ($result['translation'] && $result['translation'] != $text) {
                        ...
                        $user_id = auth()->id();
                        if (empty($user_into) 
                            || empty($user_into[$user_id]) 
                            || $user_into[$user_id] != $target
                        ) {
                            $user_into[$user_id] = $target;
                            \Option::set('tickettranslator.into', $user_into);
                        }
