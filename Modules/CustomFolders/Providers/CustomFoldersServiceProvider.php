...
class CustomFoldersServiceProvider extends ServiceProvider
{
    ...
    public function hooks()
    {
        ...
        \Eventy::addFilter('folder.conversations_query', function($query, $folder, $user_id) {
            ...
            if (!empty($folder->meta['unassigned'])) {
            	$query->where('conversations.user_id', null);
            } elseif (!empty($folder->meta['own_only'])) {
        ...
        \Eventy::addFilter('mailbox.folders', function($folders) {
            foreach ($folders as $i => $folder) {
                ...
                if (!empty($folder->user_id) && $folder->user_id != $user_id) {
                    // If user_id is a Team.
                    $show_to_team = false;
                    
                    if (\Module::isActive('teams')
                        && \Team::isTeam($folder->user)
                        && in_array($user_id, \Team::getMemberIds($folder->user))
                    ) {
                        $show_to_team = true;
                    }
                    if (!$show_to_team) {
        ...
        // https://github.com/freescout-helpdesk/freescout/issues/3988
        \Eventy::addFilter('conversation.get_nearby_status', function($status_applied, $query, $status, $conversation, $folder) {
            if (isset($folder->meta['status_filter']) 
                && !empty($folder->meta['status_filter']) 
