@if(isset($template))
    ...
    @if (isset($template['params']))
        @foreach ($template['params'] as $pk=>$p)
            <div class="wtparams {{ $p }}">{{ $pk }}: <input type="text" class="form-control wf-email-input" name="wt_params_{{ $pk }}"/></div>
        @endforeach
    @endif
    ...
    @if(isset($template['header']['need_files']))
        ...
        <div id="wt-header-files">
            <input type="hidden" class="form-control wf-email-input" name="wt_need_files" value="1">
            ...
            <div class="wt_files-or-example_block @if(!$hasExample) wt-hidden @endif">
                ...
                <input type="hidden" class="wf-email-input" name="wt_file_type" value="@if($hasExample) 0 @else 1 @endif">
            </div>
            ...
            @if(isset($template['header']['files']))
                <div id="block-view-files" class="@if(!$hasExample) wt-hidden @endif">
                    <input type="hidden" name="wt_has_files_example" class="wf-email-input" value="1">
                    @foreach($template['header']['files'][$template['header']['format']] as $key => $link)
                        <input type="hidden" name="wt_old_files_{{ $key }}" class="wf-email-input" value="{{ $key }}">
