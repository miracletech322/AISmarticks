    ...
    @foreach ($customer_fields as $customer_field)
        <div class="form-group{{ $errors->has($customer_field->getNameEncoded()) ? ' has-error' : '' }}">
            ...
            <div class="col-sm-6">
                ...
                @elseif ($customer_field->type == CustomerField::TYPE_MULTISELECT)
                    {{-- Add hidden input to allow saving empty value --}}
                    <input type="hidden" name="{{ $customer_field->getNameEncoded() }}[]" value="" />
                    <select class="form-control input-sized-lg @if (!$customer_field->required && !$customer_field->value) placeholdered @endif   crm-cf-multiselect" name="{{ $customer_field->getNameEncoded() }}[]" multiple @if ($customer_field->required) required @else  placeholder="{{ __('(optional)') }}" @endif>
                        
                        @foreach ($customer_field->getMultiselectValues() as $value)
