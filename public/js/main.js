...
var EditorInsertVarButton = function (context) {
	...
	var vars = {
		...
		customer: {
			...
			'customer.phone': Lang.get("messages.phone"),
		},
		user: {
...
$(document).ready(function(){
	...
	$(".billing_monthheader").click(function(){
		$(".billing_month").hide();
		$(".billing_month.month"+$(this).attr("month")).show();
		return false;
	});
	$(".billing_month").hide();
...
function switchToNewEmailConversation()
{
	...
	$('#sms-conv-switch').removeClass('active');
	$('#whatsapp-conv-switch').removeClass('active');
	$('.email-conv-fields').show();
	$('.phone-conv-fields').hide();
	$('.sms-conv-fields').hide();
	$('.whatsapp-conv-fields').hide();
	$('.whatsapp-templates-conv-fields').hide();
	// $('.conv-reply-body').show();
	viewSubmitButton(false)
	$('.subject-conv-fields').show();
	...
	$('#form-create :input[name="is_sms"]:first').val(0);
	$('#form-create :input[name="is_whatsapp"]:first').val(0);
...
function switchToNewSmsConversation()
{
	$('#email-conv-switch').removeClass('active');
	$('#phone-conv-switch').removeClass('active');
	$('#sms-conv-switch').addClass('active');
	$('#whatsapp-conv-switch').removeClass('active');
	$('.email-conv-fields').hide();
	$('.phone-conv-fields').hide();
	$('.custom-conv-fields').hide();
	...
	$('#form-create :input[name="is_note"]:first').val(0);
	$('#form-create :input[name="is_phone"]:first').val(0);
	$('#form-create :input[name="is_sms"]:first').val(1);
	$('#form-create :input[name="is_whatsapp"]:first').val(0);
	$('#form-create :input[name="type"]:first').val(Vars.conv_type_chat);
...
function switchToNewWhatsappConversation()
{
	$('#email-conv-switch').removeClass('active');
	$('#phone-conv-switch').removeClass('active');
	$('#sms-conv-switch').removeClass('active');
	$('#whatsapp-conv-switch').addClass('active');
	$('.email-conv-fields').hide();
	$('.phone-conv-fields').hide();
	$('.custom-conv-fields').hide();
	$('.sms-conv-fields').hide();
	$('.whatsapp-conv-fields').show();
	$('.whatsapp-templates-conv-fields').hide();
	// $('.conv-reply-body').hide();
	viewSubmitButton(true)
	$('.subject-conv-fields').hide();
	...
	$('#form-create :input[name="is_phone"]:first').val(0);
	$('#form-create :input[name="is_sms"]:first').val(0);
	$('#form-create :input[name="is_whatsapp"]:first').val(1);
	$('#form-create :input[name="type"]:first').val(Vars.conv_type_chat);

	if ($('#whatsappphone').val()) {
		whatsappTemplateOrSimple()
	}

	// alert('eee')
	// $('.whatsapp-template-send').click(function () {
	// 	$('#form-create').submit()
	// })
...
function initNewConversation(is_phone)
{
    $(document).ready(function() {
		...
		$(document).on('click', '#wt-header-files .wt_files-or-example_block button', function () {
			changeWTFileType($(this))
		})

		$(document).on('click', '#block-view-files .wt-document .wt-save-file', function () {
			wtDownloadFile($(this))
		})

		$("#whatsappphone").change(function(){
			whatsappTemplateOrSimple()
		});

		$("#whatsapptemplate").change(function(){
			whatsappViewTemplate()
		});

		$("#whatsapptemplate").change(function(){
			$(".wtemplate").hide();
			$("#wtemplate_"+$(this).val()).show();
		});
		$(".wtemplate").hide();
		$("#wtemplate_"+$("#whatsapptemplate").val()).show();
		...
		$('#sms-conv-switch').click(function() {
			switchToNewSmsConversation();
		});
		$('#whatsapp-conv-switch').click(function() {
			switchToNewWhatsappConversation();
		});

		$(document).on('click', '#block-view-files .wt-remove-file', function() {
			removeWTFile($(this));
		});
...
function initReplyForm(load_attachments, init_customer_selector, is_new_conv)
{
	$(document).ready(function() {
	    ...
	    $(".btn-reply-submit").click(function(e) {
			...
			if (!($('#whatsapp-conv-switch').hasClass('active') && $('#whatsapp-type-of-message').val() == 'template')) {
			...
			var formData = new FormData(form[0]); //test
			// data = form.serialize();
	    	// data += '&action=send_reply';
			formData.append('action', 'send_reply'); // test
			// formData.append('data', data); // test
			...
			fsAjax(formData, laroute.route('conversations.ajax'), function(response) {
					if (typeof(response.status) != "undefined" && response.status == 'success') {
						// Forget note
						if (is_note) {
...
function whatsappTemplates()
{
	$(document).on('click', '.template-delete', function () {
		...
		var mailbox_id = $('#templates-table').data('mailbox')
		var btn = $(this)

		if (confirm('Do you really want to delete the template with the name: ' + name + ' ?')) {
			$.ajaxSetup({
				headers: {
			...
			$.ajax({
				url: '/whatsapp/remove-template', // URL, куда отправляется запрос
				method: 'POST', // Метод запроса (GET, POST, PUT, DELETE и т.д.)
				data: { // Данные, которые отправляются на сервер (если нужно)
					name: name,
					mailbox: mailbox_id,
				},
				success: function(response) { // Обработчик успешного ответа
					console.log(response)
					var html = '';
					if (response == 'success') {
						btn.closest('tr').slideUp(200)
						html = "<div class='template_info-alert alert alert-success'>\n" +
							"        	<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Template message successfully deleted!\n" +
							"    	</div>";
					}else if(response == 'error') {
						html = "<div class='template_info-alert alert alert-danger' style='display:block;'>\n" +
							"        	<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Template message could not be deleted!\n" +
							"    	</div>";
					}

					if (html) {
						if ($('.template_info-alert').length) {
							$('.template_info-alert').remove()
						}
						$('#app .layout-2col .content-2col .col-xs-12').prepend(html)
						$('#app .layout-2col .content-2col .col-xs-12').scrollTop(0)
					}
				},
				error: function(xhr, status, error) { // Обработчик ошибки
					var html = "<div class='template_info-alert alert alert-danger'>\n" +
						"        	<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Template message could not be deleted!\n" +
						"    	</div>";
...
{
	$(document).on('click', '.template_components .component_types button', function () {
		var type = $(this).data('component-type')

		$('.template_components .component_types button').removeClass('active')
		$(this).addClass('active')
		$('.template_components .component').slideUp(200)
		$('.template_components .component[data-component-type-block='+ type +']').slideDown(200)
	})

	$(document).on('change', '#components_format', function () {
		var format = $(this).val()
		if (format == 'TEXT') {
			$('.form-group.components_header_files').slideUp(200)
			$('#components_header-text').closest('.form-group').slideDown(200)
		}else {
			$('#components_header-text').closest('.form-group').slideUp(200)
			$('.form-group.components_header_files').slideDown(200)
		}
	})
}

function whatsappTemplateOrSimple() {
	form = $(".form-reply:first");
	data = form.serialize();
	data += '&action=whatsapp_check';
	fsAjax(data, laroute.route('conversations.ajax'), function(response) {
			console.log(response)
			if (!$('#whatsapp-type-of-message').length) {
				form.append("<input type='hidden' name='wt_type' id='whatsapp-type-of-message'>")
			}
			if (response.wlastok)
			{
				$(".whatsapp-templates-conv-fields").hide();
				...
				$('#whatsapp-type-of-message').val('simple')
				viewSubmitButton(false)
			}
			else
			{
				whatsappViewTemplate()
				$(".whatsapp-templates-conv-fields").show();
				// $('.conv-reply-body').hide();
				$('.subject-conv-fields').hide();
				$("#whatsapptemplate").show();
				$('#whatsapp-type-of-message').val('template')
				viewSubmitButton(true)
			}
		},
		true,
...
function viewSubmitButton($is_whatsapp=false) {
	var container = $('.conv-reply-body');

	if ($is_whatsapp) {
		container.find('.note-toolbar-wrapper.panel-default').hide()
		container.find('.note-editing-area').hide()
		container.find('#editor_signature').hide()
	}else {
		container.find('.note-toolbar-wrapper.panel-default').show()
		container.find('.note-editing-area').show()
		container.find('#editor_signature').hide()
	}
}

function whatsappViewTemplate() {
	var mailbox = $('body').data('mailbox_id')
	var template_id = $('#whatsapptemplate').val()
	...
	$.ajax({
		url: '/conversation/get-template', // URL, куда отправляется запрос
		method: 'POST', // Метод запроса (GET, POST, PUT, DELETE и т.д.)
		data: { // Данные, которые отправляются на сервер (если нужно)
			mailbox: mailbox,
		...
		success: function(response) { // Обработчик успешного ответа
            $('#wt-form').html(response)

		},
		error: function(xhr, status, error) { // Обработчик ошибки
			console.log('error get template')
		}
	});
}

function removeWTFile(el) {
	var container = el.closest('.wt-files_container')
	var file_id = container.data('file-id')

	$('input[name="wt_old_files[]"][value="' + file_id + '"]').remove()
	container.slideUp(200)
	setTimeout(function () {
		container.remove()
		if (!$('#block-view-files .wt-files_container').length) {
			$('.wt_files-or-example_block button[data-file-type="0"]').remove()
			$('.wt_files-or-example_block button[data-file-type="1"]').click().remove()
		}
	}, 210)
}

function changeWTFileType(btn) {
	var type = btn.data('file-type')
	var container = btn.closest('.wt_files-or-example_block')
	var filesBlock = $('#block-view-files')
	var newFilesBlock = $('.wtparams.wt-new-file')

	$('.wt_files-or-example_block button').removeClass('active')
	btn.addClass('active')
	container.find("input[name='wt_file_type']").val(type)

	if (type == 0) {
		filesBlock.removeClass('wt-hidden')
		newFilesBlock.addClass('wt-hidden')
	}

	if (type == 1) {
		newFilesBlock.removeClass('wt-hidden')
		filesBlock.addClass('wt-hidden')
	}
}

function wtDownloadFile(el) {
	var link = el.data('wt-doc-link')
...
function wfWhatsappViewTemplate() {
	var mailbox = $('body').data('mailbox_id')
	var template_id = $('#wfwhatsapptemplate').val()
	...
	$.ajax({
		url: '/whatsapp/get-template', // URL, куда отправляется запрос
		method: 'POST', // Метод запроса (GET, POST, PUT, DELETE и т.д.)
		data: { // Данные, которые отправляются на сервер (если нужно)
			mailbox: mailbox,
			templateId: template_id,
...
$("body").on("change","#wfwhatsapptemplate",function(){
	wfWhatsappViewTemplate();
});

$("body").on("click",".prev_conv_more_extender",function(){
	$(this).hide();
	$(this).closest('.prev_conv_more_container').find(".prev_conv_less_extender").show();
	$(this).closest('.prev_conv_more_container').find('.prev_conv_more').show();
	// $("#conv-layout-customer").css("overflow","auto");
	adjustCustomerSidebarHeight();
	setTimeout(adjustCustomerSidebarHeight, 2000);
	return false;
});
$("body").on("click",".prev_conv_less_extender",function(){
	$(this).hide();
	$(this).closest('.prev_conv_more_container').find(".prev_conv_more_extender").show();
	$(this).closest('.prev_conv_more_container').find('.prev_conv_more').hide();
	// $("#conv-layout-customer").css("overflow","auto");
	adjustCustomerSidebarHeight();
	setTimeout(adjustCustomerSidebarHeight, 2000);
	return false;
});
$(".prev_conv_less_extender").hide();
