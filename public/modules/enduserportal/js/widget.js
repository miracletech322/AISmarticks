...
if (typeof(SmarticksW) != "undefined" && SmarticksW && typeof(SmarticksW.s) != "undefined") {
	...
	SmarticksW.init = function () {
        ...
        if (script) {
			SmarticksW.form_url = script.src.replace(/modules\/enduserportal\/.*/, '');
			SmarticksW.form_url += 'help/widget/form/'+SmarticksW.s.id;
			SmarticksW.form_url += '?'+Object.keys(SmarticksW.s)
                .map(function(k) {return encodeURIComponent(k) + '=' + encodeURIComponent(SmarticksW.s[k]);})
                .join('&');

            if (typeof(SmarticksW.f) != "undefined") {
				SmarticksW.form_url += '&'+Object.keys(SmarticksW.f)
                    .map(function(k) {return 'f['+encodeURIComponent(k) + ']=' + encodeURIComponent(SmarticksW.f[k]);})
                    .join('&');
            }
        }
